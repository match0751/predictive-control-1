...
for j=1:1:N
    ...
    for Nref=1:1:Np
        ...
        Yref(Nref,1)=3 + (j-1+Nref)*T*0.1 ;
%         Yref(Nref,1)=3 + (j-1+Nref)*T*0.1*((j-1+Nref)*T-10) ;
        PHIref(Nref,1)=atan(Yref(Nref,1)/Xref(Nref,1));
    end
...
%     [A,fval,exitflag]=fminbnd(@(x)MY_costfunction(x,State_Initial,Np,Nc,T,Xref,Yref,PHIref,Q,R),lb,ub);%只有上下界约束，但容易陷入局部最小
...
%     if j>100 && j<130 
%     XOUT=dsolve('Dx-v_actual*cos(z)=0','Dy-v_actual*sin(z)=0','Dz-v_actual*tan(deltaf_actual)=0','x(0)=X00(1)-0.03','y(0)=X00(2)-0.05','z(0)=X00(3)');
%     t=T;
%     State_Initial(1,1)=eval(XOUT.x);
%     State_Initial(2,1)=eval(XOUT.y);
%     State_Initial(3,1)=eval(XOUT.z);        
...
%     
    ...
    for Nref=1:1:N
        Xp(Nref,1)=(Nref-1)*T;
        Yp(Nref,1)=3 + (-1+Nref)*T*0.1*((-1+Nref)*T-10);
    end
%     plot(Xp,Yp,'r-.');
    hold on;
